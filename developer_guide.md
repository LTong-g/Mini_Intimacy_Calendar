...
- 黑名单应用设置页顶部已选择数量以原有边框卡片样式固定显示，卡片下方提供应用名称和包名搜索栏，搜索结果优先显示应用名匹配项且开头匹配排在前面，搜索栏下方左侧提供“全部 / 已选”列表切换；只滚动下方应用列表；应用列表中英文按拼音首字母混排，无法归入 A-Z 的应用归入末尾 #；应用列表框与上方卡片和搜索框同宽，首字母快速滑动条位于列表框外并贴近屏幕右侧边缘，触碰时高亮当前命中字母；右下角刷新按钮在重新读取应用列表时显示旋转动画。
