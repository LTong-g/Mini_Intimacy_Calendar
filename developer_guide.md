...
- 黑名单使用时间段页面只读取已保存使用时间段和已保存黑名单应用信息，不读取完整应用列表或提取主色；全部记录详情和单个应用详情的顶部汇总固定显示，只滚动下方记录，记录上方可通过今日、7 天、30 天三个按钮切换范围，右侧筛选按钮可打开筛选弹窗设置日期范围。自定义日期范围确认后，原范围按钮位置显示当前日期范围；再次打开筛选弹窗时显示当前筛选态，存在筛选时左侧操作为清除筛选。全部记录详情的筛选弹窗顶部按屏幕宽度显示当前选中应用图标预览，最右侧编辑按钮可打开应用显示筛选弹窗，在已显示和未显示应用图标之间点击移动；单个应用详情的筛选弹窗顶部居中显示当前应用图标；记录详情按日期分组显示，日期标题右侧显示该日期合计使用时长。
- 黑名单功能支持读取黑名单应用使用时间段，并按当日、最近 7 天、最近 30 天展示统计图。
...
- 黑名单功能最近 30 天趋势图中，连续多天无使用记录时只显示这段连续空心点的首尾点，中间仅保留趋势线。
