    ...
    <>
      ...
      >
        ...
        <View style={styles.colorPreviewRow}>
          ...
          <Text style={styles.hexText}>{categoryColor}</Text>
        </View>
        {['R', 'G', 'B'].map((channel, index) => {
          const rgbParts = getRgbParts(categoryColor);
          const value = rgbParts[index];
          return (
            <RgbSlider
              key={channel}
              label={channel}
              value={value}
              color={channel === 'R' ? '#FF3B30' : channel === 'G' ? '#34C759' : '#007AFF'}
              onChange={(next) => {
                const nextParts = [...rgbParts];
                nextParts[index] = next;
                setCategoryColor(buildHexColor(nextParts));
              }}
            />
          );
        })}
...
const styles = StyleSheet.create({
  ...
  colorPreviewRow: { minHeight: 40, flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  colorPreview: { width: 32, height: 32, borderRadius: 16, borderWidth: 1, borderColor: '#d8d8d8', marginRight: 10 },
  hexText: { color: '#333', fontWeight: '600' },
  sliderRow: { minHeight: 42, flexDirection: 'row', alignItems: 'center', marginTop: 8 },
  sliderLabel: { width: 22, fontSize: 13, color: '#555', fontWeight: '600' },
  sliderTrackWrap: {
    flex: 1,
    height: 32,
    justifyContent: 'center',
  ...
  sliderTrack: { height: 8, borderRadius: 4, backgroundColor: '#D1D5DB' },
  ...
  sliderValue: { width: 34, textAlign: 'right', fontSize: 13, color: '#555' },
