...
const versionHistory = [
  {
    ...
    featureIntro: {
      ...
      notes: [
        ...
        '黑名单统计可按当日、最近 7 天、最近 30 天查看使用时长图表，最近 30 天趋势会在连续多天无使用记录时只保留首尾空心点；使用时间段页面可按全部记录或单个应用查看详情，详情顶部汇总固定显示，下方记录可通过今日、7 天、30 天三个按钮切换范围，也可通过筛选弹窗设置日期范围；全部记录详情还可筛选显示哪些黑名单应用；记录按日期分组并显示当日合计时长，全部记录详情中的每条记录会显示对应应用图标。',
