    ...
    sections: [
      {
        label: '修复：',
        notes: [
          '修复黑名单应用页刷新按钮在重新读取应用列表时转动不及时的问题。',
        ],
      },
      {
        label: '新增：',
      ...
      },
      {
        ...
        notes: [
          ...
          '优化黑名单应用页，固定已选择数量、增加搜索栏、全部/已选切换、右下角刷新按钮动画和屏幕右侧首字母快速跳转，应用列表中英文按拼音首字母混排，字母条触碰时会高亮当前字母且 # 位于末尾；搜索结果会优先显示应用名匹配项且开头匹配排在前面，选择变化会实时保存，并减少统计页和使用时间段页的重复应用列表读取。',
