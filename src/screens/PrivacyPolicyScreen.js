...
const sections = [
  {
    ...
    items: [
      ...
      '黑名单功能会保存用户选择的黑名单应用列表、应用图标主色，以及读取和合并后的黑名单应用使用开始时间、结束时间和使用时长。',
      '进入黑名单主页时，应用会在已获得使用情况访问权限且已选择黑名单应用的前提下，静默读取最近三天的黑名单应用使用记录并保存到本地。',
  ...
  },
  {
    ...
    items: [
      ...
      '黑名单使用记录仅保存在当前设备本地，不会自动写入日历打卡记录。',
