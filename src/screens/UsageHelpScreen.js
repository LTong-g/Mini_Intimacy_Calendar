...
const sections = [
  ...
  },
  {
    ...
    items: [
      ...
      '黑名单的使用时间段通过独立入口查看；进入后可先选全部记录或某个应用，再查看对应记录；记录详情顶部汇总固定显示，默认显示 7 天范围，下方记录可通过今日、7 天、30 天三个按钮切换范围，右侧筛选按钮可设置日期范围；全部记录详情还可在筛选弹窗中选择显示哪些黑名单应用；记录按日期分组并在日期右侧显示当日合计时长，全部记录详情中的每条记录会显示对应应用图标。',
      '黑名单主页每次进入时会在后台静默读取最近三天使用记录，不显示动画或弹窗；也可下拉手动读取最近三天记录，或点击按日期读取记录，通过与自定义统计相同的日期选择页面设置起止日期后读取并保存；手动读取完成弹窗会显示请求范围、实际读取到的记录范围和聚合后的使用时间段数量。',
      ...
      '黑名单当日饼图会优先使用应用图标主色，最近 7 天和最近 30 天统计图使用观看教程的黄橙色系作为数据主色。',
